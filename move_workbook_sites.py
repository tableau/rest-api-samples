...
def main():
    ...
    # Default site
    source_auth_token, source_site_id = sign_in(server, username, password)

    # Specified site
    dest_auth_token, dest_site_id = sign_in(server, username, password, site=dest_site)

    ##### STEP 2: Find workbook id #####
    print("\n2. Finding workbook id of '{0}' from source site".format(workbook_name))
    workbook_id = get_workbook_id(server, source_auth_token, source_site_id, workbook_name)
